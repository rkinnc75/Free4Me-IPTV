# fix563 — TV search D-pad: arrow-up moves on the first press, every time

Base: **v2.0.62** (CI latest). Ships as **v2.0.63+563**.

## Why this exists (fix562 did not resolve it)

fix562 shipped `focusInDirection(TraversalDirection.up)` as the up handler. That
call routes through the **same** `DirectionalFocusTraversalPolicyMixin.inDirection`
pass that is unreliable right after a programmatic focus jump across the search
screen's stacked grids (flutter/flutter#70364) — so it could not fix the
first-press miss. fix563 removes that dependency entirely.

## Root cause (proven from the fix561 field log + Flutter 3.44.0 source)

The whole results area is one `ListView` of stacked, shrink-wrapped `GridView`s
sharing a single ambient focus scope, so every arrow move runs through one
`ReadingOrderTraversalPolicy` (mixes in `DirectionalFocusTraversalPolicyMixin`).
The cross-section escape lands focus with a **programmatic** `requestFocus()`,
which never updates that policy's internal directional state (`_policyData`
history + `focusedChild`/rect bookkeeping). `inDirection` then computes the next
move from that stale/teleported state, so the **first** arrow-up after a jump can
resolve to no candidate and move nothing; the second press, run against settled
state, works. The log proves it is non-deterministic: the same escape → Up
**worked** landing in "Coming up" (multi-row) but **died** landing in "On now"
(topmost, almost certainly a single row).

## Fix — stop using directional traversal for vertical moves

- Every search tile now owns a stable `FocusNode` (full per-section list, not
  just the last row as in fix561).
- `_upTargetFor` computes the up target by index, and the tile moves focus there
  unconditionally via the existing `onFocusUpEscape` hook:
  - interior / lower rows -> the node exactly one row up (`myNodes[i - columns]`);
  - a section's top row with a previous section -> that section's last row at the
    same column, clamped (the proven cross-section landing);
  - the first section's top row -> the **search field** (now owns a `FocusNode`),
    so leaving the very top is deterministic too — covers the single-row
    "On now" case, where there is no row above to move to.

No arrow-up press in search results depends on directional traversal anymore, so
the outcome can't vary with scroll/policy/rect state. Down / Left / Right are
unchanged (never reported broken; rect-based, unaffected by the new node
ownership). The fix562 `focusInDirection(up)` fallback is removed; fix560's
`ensureVisible` (200 ms) is kept.

Verified in-sandbox: `git apply --check` passes on a fresh `v2.0.62` clone;
brace/paren/bracket balance OK; no orphaned identifiers; index math checked by
hand (incl. short last rows and cross-section column clamping). `flutter analyze`
(0 INFOs) and `flutter test` to be confirmed on the build machine.

Files: `lib/channel_tile.dart`, `lib/tv/tv_search_view.dart`,
`lib/whats_new_modal.dart`, `pubspec.yaml`.

```diff
diff --git a/lib/channel_tile.dart b/lib/channel_tile.dart
index 10b8517..f464a4b 100644
...
+  /// aligned with where the user came from and makes every press deterministic.
   List<Widget> _buildShelves() {
     final groups = <(String, List<Channel>)>[
       ('On now', _onNow),
@@ -322,20 +328,59 @@ class _TvSearchViewState extends State<TvSearchView> {
     return sections;
   }
...
git commit -m "fix563: TV search D-pad up moves on first press via node-reference navigation"

```bash
bash scripts/build_and_release.sh v2.0.63
```
