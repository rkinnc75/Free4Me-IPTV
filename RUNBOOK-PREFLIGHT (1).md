...
6. **Wrapping a widget = ONE atomic edit, never two.** Adding a wrapper (e.g.
   `FocusableActionDetector` around a `GestureDetector`, or `DpadFocusEscape` around a
   field) changes the brace/paren count at BOTH ends. Doing it as two edits — change the
   opener here, add a closing brace there — is how you get a stray `;`/`}` and a build
   break (this happened twice: setup.dart wrappers, and the fix170 `_buildVideoCell`
   split that emitted `expected ';' … dead_code` at line 859). **Replace the entire
   enclosing method/widget as a single block** so the closes are counted once, together.
   When practical, **ship the whole corrected file** rather than a diff.

7. **Verify brace/paren/bracket balance before shipping any structural edit.** The author
   has no compiler, but balance is checkable: write the edited file to disk and run a
   Dart-aware bracket checker (one that understands `//`, `/* */`, `'...'`/`"..."`, raw
   strings, escapes, and `${...}` interpolation **including nested quotes inside it** —
   a naive checker desyncs on `'${x ? "A" : "B"}'`). Validate the checker by confirming
   it passes the **known-good original** file, then run it on the edited file. If it can't
   pass the original, the checker is wrong — fix the checker, don't trust its verdict.

8. **The deliverable is ONE file: the `fix###.md` runbook. No companion files by default.**
   The runbook must be self-contained — every change expressed as exact current→replacement
   blocks the build machine applies itself. Do NOT ship pre-edited `.dart` (or other source)
   files alongside it as the normal path. One runbook in, one fix out.
   - **Why:** extra files create ambiguity (which is authoritative — the runbook block or
     the attached file?), can drift from each other, and turn a clean "apply fix###.md"
     handoff into a multi-file reconciliation. The runbook is the single source of truth.
   - **Special-circumstances exception (must state the reason in the runbook):** ship a
     whole source file ONLY when a diff is genuinely unsafe or unwieldy — e.g. a large
     atomic structural rewrite where brace-balance across the whole method/file matters
     (see rule 6/7) and an in-text block would be too error-prone to hand-apply. Even then:
     (a) note explicitly in the runbook why the file is attached, (b) still describe the
     change in the runbook so it's reviewable, and (c) keep it to the single file that
     needs it — never a pile of attachments. If you're attaching more than one source file,
     that's a signal the fix is too broad; split it or reconsider.
   - Default expectation for the build machine: if a runbook ships extra files without a
     stated special-circumstances reason, treat the runbook blocks as authoritative.
