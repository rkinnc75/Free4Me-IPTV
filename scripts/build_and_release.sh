...
#   5. Pushes to github.com:rkalsky/Free4Me-IPTV (using ~/.ssh/id_rsa)
#   6. Creates a GitHub Release tagged v{version}
#   7. Uploads the APK as a release asset
#
# Prerequisites:
#    GitHub token stored in macOS Keychain:
#       security add-internet-password -s api.github.com -a rkalsky -w <token>
...
GITHUB_TOKEN=$(security find-internet-password -s "api.github.com" -a "rkalsky" -w 2>/dev/null)
if [[ -z "$GITHUB_TOKEN" ]]; then
  echo "ERROR: GitHub token not found in keychain." >&2
  echo "Run: security add-internet-password -s api.github.com -a rkalsky -w <token>" >&2
  exit 1
fi
...
EXISTING=$(curl -sf \
  ...
  "https://api.github.com/repos/rkalsky/Free4Me-IPTV/releases/tags/$TAG" \
...
See [DEVELOPMENT-HANDBOOK.md](https://github.com/rkalsky/Free4Me-IPTV/blob/main/DEVELOPMENT-HANDBOOK.md) for the full feature roadmap and changelog."
...
PYEOF
  ...
  RESPONSE=$(curl -sf -X POST \
    ...
    "https://api.github.com/repos/rkalsky/Free4Me-IPTV/releases" \
    -d "$JSON_PAYLOAD")
...
EXISTING_ASSET_ID=$(curl -sf \
  -H "Authorization: token $GITHUB_TOKEN" \
  "https://api.github.com/repos/rkalsky/Free4Me-IPTV/releases/${RELEASE_ID}/assets" \
  | python3 -c "
import sys, json
...
if [[ -n "$EXISTING_ASSET_ID" ]]; then
  ...
  curl -sf -X DELETE \
    -H "Authorization: token $GITHUB_TOKEN" \
    "https://api.github.com/repos/rkalsky/Free4Me-IPTV/releases/assets/${EXISTING_ASSET_ID}" || true
fi
...
UPLOAD_RESPONSE=$(curl -sf -X POST \
  ...
  "https://uploads.github.com/repos/rkalsky/Free4Me-IPTV/releases/${RELEASE_ID}/assets?name=${APK_NAME}")
...
echo "  https://github.com/rkalsky/Free4Me-IPTV/releases/tag/${TAG}"
echo ""
