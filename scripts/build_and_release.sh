...
#   3. Copies 4 APKs (arm/arm64/x64/universal) to ~/Downloads/
...
#   7. Uploads all four APKs as release assets
#
# Prerequisites:
...
# fix692: four artifacts — three per-ABI slims + one universal (all ABIs).
# The universal is the first-install default and the legacy apkUrl target.
APK_NAME="Free4Me-IPTV-${VERSION}-universal.apk"
APK_ARM="Free4Me-IPTV-${VERSION}-arm.apk"
APK_ARM64="Free4Me-IPTV-${VERSION}-arm64.apk"
APK_X64="Free4Me-IPTV-${VERSION}-x64.apk"
TAG="v${VERSION}"
...
# fix692: per-ABI splits (arm/arm64/x64) + one universal APK, mirroring
# .github/workflows/release.yml. fix504 history: arm64-only broke 32-bit-only
# TV boxes (onn 4K etc.), so the universal remains the safe default.
flutter build apk --release --split-per-abi \
  --target-platform android-arm,android-arm64,android-x64
flutter build apk --release --target-platform android-arm,android-arm64,android-x64

APK_OUT="$REPO_DIR/build/app/outputs/flutter-apk"

cp "$APK_OUT/app-release.apk"           "$HOME/Downloads/$APK_NAME"
cp "$APK_OUT/app-armeabi-v7a-release.apk" "$HOME/Downloads/$APK_ARM"
cp "$APK_OUT/app-arm64-v8a-release.apk"   "$HOME/Downloads/$APK_ARM64"
cp "$APK_OUT/app-x86_64-release.apk"      "$HOME/Downloads/$APK_X64"
echo " APKs copied to ~/Downloads/ ($APK_NAME + arm/arm64/x64 slims)"
...
# fix692: the old inline Python heredoc duplicated (and had drifted from)
# scripts/update_version_json.py — which is also the single implementation CI
# uses and the one that writes the per-ABI apkUrls map. Call it instead.
...
#  8. Upload APK assets (fix692: all four)
for NAME in "$APK_ARM" "$APK_ARM64" "$APK_X64" "$APK_NAME"; do
...
" 2>/dev/null || true)
  ...
  UPLOAD_RESPONSE=$(curl -sf -X POST \
    ...
    --data-binary @"$HOME/Downloads/$NAME" \
    "https://uploads.github.com/repos/rkinnc75/Free4Me-IPTV/releases/${RELEASE_ID}/assets?name=${NAME}")
  echo "   $(echo "$UPLOAD_RESPONSE" | python3 -c "import sys,json; print(json.load(sys.stdin)['browser_download_url'])")"
done
