# fix381 — Add Source is now a single screen, not a 4–5 step wizard

**Target version:** `1.34.10+381` (from `1.34.9+380`)
**Branch:** `main`
**Toolchain (verified in sandbox):** Flutter 3.44.0 / Dart 3.12.0 (CI pin), `libsqlite3.so` symlinked.
**Written against:** `v1.34.9+380` on main.

## What & why

`lib/setup.dart` was a 4–5 screen Add Source wizard: after Welcome
and Source Type, the user was stepped through Name → URL → Username →
Password → EPG URL one screen at a time, with a "Next" button between
each. This made adding a single Xtream source a 7-screen journey
(Welcome → Type → Name → URL → Username → Password → EPG → Finish)
and a small M3U file add a 4-screen journey.

The fields don't depend on each other — the user knows all of them
when they're adding a source. The wizard just delayed the moment
they could enter them all together.

fix381 collapses everything past the source-type pick into a single
form page. The user picks the type, sees every field for that type
on one screen, and taps "Add Source" once. EPG URL is folded into
the same screen (was a separate step). Submit button is labelled
"Add Source" on the form (was "Next" → EPG → "Add Source" on the
old multi-step flow).

## Files changed
- `lib/setup.dart` — rewritten. Single `FormBuilder` for all fields
  with conditional rendering per source type. `Steps` enum now 4
  values (was 8). Controllers and focus nodes are now per-field,
  not per-step. New `_pickedM3uPath` state for the M3U file picker
  (was: auto-pick on the Name step's "Next" tap). Welcome, source
  type, finish, import-backup, and receive-via-QR pages are
  unchanged.
- `lib/models/steps.dart` — enum collapsed to
  `welcome, sourceType, form, finish` (was 8 values).
- `lib/whats_new_modal.dart` — `'1.34.10'` changelog entry (real UTF-8).
- `pubspec.yaml` — `1.34.9+380` → `1.34.10+381`.
- `test/setup_form_test.dart` — **new**: 6 widget tests on the
  single-screen form. First widget test in the suite — pattern
  reference for future setup-flow tests.

## Design decisions confirmed before coding

- **EPG URL folded into the same screen** (not a separate step).
- **Submit button label:** "Add Source" on the form, "Finish" on
  the finish page, "Next" on welcome/sourceType.

## Evidence (run in sandbox on fresh main + patch)
- `flutter analyze --no-fatal-infos` → **No issues found!**
- `flutter test` → **All tests passed! (62)** (56 baseline + 6 new).
...
1. `grep -c '^version: 1.34.10+381' pubspec.yaml` → **1**
2. `grep -n "'1.34.10':" lib/whats_new_modal.dart` → one hit
3. `grep -c "enum Steps" lib/models/steps.dart` → **1**
4. `grep -E "Steps\.(name|url|username|password|epgUrl)" lib/ -r` → **0** (all 5 old enum members removed)
5. `grep -c "case Steps.form:" lib/setup.dart` → **1** (the new single-screen form page)
6. `test -f test/setup_form_test.dart` → present
7. `flutter analyze --no-fatal-infos` → `No issues found!`
8. `flutter test` → `All tests passed!` (**62**)

## On-device integration check (REQUIRED — D-pad focus order is not sandbox-provable)

**Primary target: an Android TV** (Shield, onn 4K Plus, or any
D-pad-controlled TV form factor). The handoff's D-pad analysis
concluded that the new form should be D-pad-navigable, but the
cross-subtree traversal (last field → "Add Source" button) and
the wrap behavior of the form's `FocusTraversalGroup` are
predicted, not proven.

### A. Single-screen form D-pad flow (TV)

1. Install `v1.34.10+381` on an Android TV.
2. Open the app, navigate to **Settings → Sources → +** (or first-run
   setup if no sources yet).
3. Pick source type "Xtream". Confirm focus lands on the "Next"
   button.
4. Press D-pad Center. Form should appear, focus on the Name field.
5. D-pad Down through the fields: Name → URL → Username → Password
   → EPG URL (optional) → "Add Source" button. Each press should
   move focus to the next field/widget in that order.
6. Type into Name (use a paired Bluetooth keyboard or the on-screen
   keyboard). Confirm the field accepts input.
7. Press D-pad Up from each field; focus should walk back up the list.
8. Press D-pad Down from the "Add Source" button — focus should
   wrap to the "Back" button (or stay on "Add Source" if no wrap;
   either is acceptable per the handoff).
9. ✓ if D-pad navigation reaches every field in a predictable order
   without getting stuck.

### B. Form validation (any device)

1. On the Xtream form, leave all fields empty. Confirm the "Add
   Source" button is disabled.
2. Fill only Name. "Add Source" still disabled.
3. Fill URL too. Still disabled.
4. Fill all four. "Add Source" enables.
5. ✓ if the button enables only when all required fields are
   non-empty.

### C. Multi-source behaviour preserved (TV or phone)

1. Add a source via the new single-screen form.
2. Open Settings → Sources, confirm the new source appears with the
   name you entered.
3. ✓ if the source was added with all fields persisted (URL,
   username, password, EPG URL).

### D. M3U file picker (phone or TV with file manager)

1. Pick source type "M3U" (the file-based one).
2. Confirm the form shows "M3U file" picker and an EPG URL field,
   no URL/username/password.
3. Tap the picker. The system file picker should appear.
4. Pick an .m3u / .m3u8 file. The picker field should update with
   the file name.
5. ✓ if the picked file is shown and the form's "Add Source"
   button enables.

## Release — EXECUTE (main-only; CI owns version.json — do NOT run update_version_json.py before tagging; tags immutable)
git commit -m "fix381: collapse Add Source wizard to a single form page (name + url/credentials/file + EPG, conditional per source type); v1.34.10+381"

---

## Patch (verified: applies on fresh `main`, analyze clean, 62/62)
```diff
diff --git a/lib/models/steps.dart b/lib/models/steps.dart
index 8d781a8..d6cbf14 100644
--- a/lib/models/steps.dart
+++ b/lib/models/steps.dart
@@ -1 +1,4 @@
...
@@ -856,6 +755,141 @@ class _SetupState extends State<Setup> {
     }
   }
...
+version: 1.34.10+381
 ...
 environment:
   sdk: ^3.10.7
diff --git a/test/setup_form_test.dart b/test/setup_form_test.dart
new file mode 100644
index 0000000..1ae6a31
--- /dev/null
+++ b/test/setup_form_test.dart
@@ -0,0 +1,149 @@
