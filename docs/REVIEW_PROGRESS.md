...
- 🟡 **6. xmltv_parser.dart** — MOSTLY DONE (commit 1f9c04f): 47+48 (backpressure/cancel), 51 (window overlap), 49 (host-only errors), 50 (allowMalformed), 52+53 (_parseXmltvTime). [45 done in app_logger.] **PENDING: 46** — stall→truncation: change parse() return type to XmltvParseResult{channelMap,truncated,inserted}; update ONLY caller epg_service.downloadAndParseEpg to skip deleteStalePrograms + log 'partial/timeout' when truncated. DO THIS FIRST in the epg_service pass (file 7). DEVICE-VERIFY LATER: 47 (heap plateau on 150k feed), 50 (real non-UTF-8 feed).
...
## NEXT UP
**Wave 2 file 6 = xmltv_parser.dart** (46 stall→fail [coordinated w/ epg_service.downloadAndParseEpg
caller], 47+48 backpressure/cancel = one _maybeUngzip edit, 51 window filter overlap, 49 host-only
exception, 50 allowMalformed [safe], 52+53 _parseXmltvTime same method). [45 done in app_logger.]
Then epg_service.dart (7), player.dart (8), db_factory (9), xtream (10), tv_guide_view (11),
multi_view_cell (12), overlay_player_widget (13), mpv_engine (14), home (15), tv_search_view (16),
CastPlugin.kt (17), MainActivity.kt (18), multi_view_screen (19), stalker_xmltv_cookie_auth (20),
epg_channel_mapping (21), fix572 test (22). See the WAVE 2 table above for per-file finding IDs.

Commits so far this run: ac8fa4d, d5b59c5, b6d36a4, 20ea085 (Wave 1); af42fca (m3u),
1f9c04f (xmltv). Analyze clean, 292 tests green. Nothing pushed.
Wave 2 progress: 2 of 18 files done (m3u ✅, xmltv 🟡 #46 pending). 16 files + #46 remain.
RESUME: do xmltv #46 as the FIRST step of the epg_service (file 7) pass, then epg_service's
own findings (36,37,38,39,41,42,43,44).
