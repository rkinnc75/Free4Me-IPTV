# fix313 — Condense README + refresh with latest features → v1.26.29+313
...
## What this does
Docs + release-pipeline smoke test. Rewrites `README.md` from 157 → 94 lines (~40% shorter), keeping every section but tightening prose, and folds in features added since it was last updated: favorite categories, category links in the long-press menu, unified long-press for movies/series, opening a category showing all channels, provider-unnamed-category auto-naming, per-category VOD back-fill, unified `is_adult` safe mode, scanner-from-visible-tile + persistent reapply, source-disable gating, LAN export bundle (dump+log+settings+zip), and in-app auto-update. No code logic change.

## Verified before delivery
- `flutter analyze --no-fatal-infos` → 2 tolerated `settings_view.dart` INFOs, no warnings/errors.
- Built against live main (1.26.28+312); patch applies cleanly to a fresh clone.
- README still contains all original sections (Sources, Channels/Movies/Series, Player, Multi-view, EPG, Backup & Restore, Settings, Android TV, Updates, Installation, Building, Credits).

---

## A. Apply this patch (preferred) — `git apply fix313.patch`
```diff
diff --git a/README.md b/README.md
index 0e1205c..ecd698b 100644
--- a/README.md
+++ b/README.md
@@ -1,129 +1,74 @@
 # Free4Me-IPTV
...
## B. Files changed
1. `README.md` — condensed + feature refresh.
2. `lib/whats_new_modal.dart` — 1.26.29 changelog.
3. `pubspec.yaml` — 1.26.29+313.

## Verification before tagging
1. `flutter analyze --no-fatal-infos` → exactly **2 issues**. Else STOP.
2. `grep '^version:' pubspec.yaml` → `version: 1.26.29+313`.
3. `grep -c "  '1.26.29':" lib/whats_new_modal.dart` → `1`.
4. No migration. README-only content change otherwise.
5. **Also serves as a pipeline smoke test:** this is the first release after fix312 (version.json-after-APK reorder). In the Actions log, confirm "APK confirmed live" prints BEFORE "version.json committed to main," and that v1.26.29 ends with its APK asset present and `main/version.json` showing 1.26.29. If the reorder misbehaves, set `APK_BEFORE_VERSIONJSON: 'false'` in release.yml to roll back.
...
git apply fix313.patch
flutter analyze --no-fatal-infos      # 2 tolerated INFOs
grep '^version:' pubspec.yaml          # 1.26.29+313
grep -c "  '1.26.29':" lib/whats_new_modal.dart   # 1
git config core.hooksPath              # .githooks
git add -A
git commit -m "fix313: condense README and refresh with latest features (1.26.29)"
git push origin main
./scripts/build_and_release.sh
```
```bash
git fetch origin --tags
git merge-base --is-ancestor v1.26.29 origin/main && echo "v1.26.29 IS on main — GOOD" || echo "NOT on main — STOP"
git show origin/main:pubspec.yaml | grep '^version:'      # 1.26.29+313
git checkout v1.26.29 && python3 scripts/update_version_json.py
