# fix562 — TV search D-pad: first Up after a section-jump now moves on press one

Base: **v2.0.61** (CI latest). Ships as **v2.0.62+562**.

## Root cause (from the fix561 field log)

The cross-section escape `requestFocus()`es the previous section's last-row tile.
Focus lands correctly (`hasFocus=true`), then fix560's `Scrollable.ensureVisible`
post-frame callback re-centres that tile (alignment 0.5), changing its on-screen
rect. The landed tile's Up handler returned `KeyEventResult.ignored`, so the
**first** Up was resolved by Flutter's default `FocusTraversalPolicy` against the
tile's **pre-scroll cached geometry** — found no in-band candidate, moved nothing.
The second Up ran against settled geometry and worked. "On now / Coming up" are
immune because their targets are not `ensureVisible`-recentred.

Log evidence: after each `TvSearch escape … targetHasFocus=false` →
`focus GAINED … hasFocus=true` on the target, the next `UP … hasFocus=true` logged
`no escape, falling through to default traversal` with **no** subsequent
`focus GAINED` — i.e. the event arrived at the correct tile, which had focus, yet
default traversal produced no move. Frame/wall-clock timing already ruled out
(fix561, 3-second-gap test).

## Fix

Handle Up explicitly in `ChannelTile.onKeyEvent`, mirroring the existing
Right-arrow branch: fire `onFocusUpEscape` when present, otherwise drive the move
with `FocusScope.of(context).focusInDirection(TraversalDirection.up)` and return
`handled`. This is geometry-agnostic on the first press (does not depend on the
default traversal pass reading a stale rect). Diagnostic `AppLog` lines from
fix561 are removed.

Verified: `git apply --check` passes on a fresh `v2.0.61` clone. `flutter analyze`
(0 INFOs) and `flutter test` to be confirmed on the build machine.

```diff
diff --git a/lib/channel_tile.dart b/lib/channel_tile.dart
index b539037..10b8517 100644
--- a/lib/channel_tile.dart
+++ b/lib/channel_tile.dart
@@ -143,24 +143,25 @@ class _ChannelTileState extends State<ChannelTile> {
         }
         return KeyEventResult.handled;
       }
-      // fix562 DIAGNOSTIC: log every key event this tile's node receives, and
-      // exactly which branch handles it — to find why the FIRST Up after a
-      // cross-section escape only scrolls without moving focus, even with a
-      // 3-SECOND gap before the press (ruling out any frame/microtask timing
-      // theory — fix561's Duration.zero change is REVERTED here pending this
-      // data, since it could not have been the actual cause).
-      if (event is KeyDownEvent && event.logicalKey == LogicalKeyboardKey.arrowUp) {
-        AppLog.info('ChannelTile.onKeyEvent: UP on "${widget.channel.name}" '
-            'hasUpEscape=${widget.onFocusUpEscape != null} '
-            'nodeHash=${_focusNode.hashCode} hasFocus=${_focusNode.hasFocus}');
+      // fix562: handle Up explicitly, mirroring the Right-arrow branch above.
+      // When a tile has a cross-section escape, fire it. Otherwise drive the
+      // upward move ourselves via focusInDirection() instead of returning
+      // `ignored` and relying on the default traversal pass. The default pass
+      // computes from the tile's cached pre-scroll geometry; right after a
+      // cross-section escape lands focus on a last-row tile, fix560's
+      // ensureVisible() re-centres that tile, so the first Up resolved against
+      // stale geometry and moved nothing (the "needs a second press" bug —
+      // 4 attempts: 558/559/560/561). Driving the move here is geometry-
+      // agnostic on the first press. On now / Coming up never hit this because
+      // their targets aren't ensureVisible-recentred.
+      if (event is KeyDownEvent &&
+          event.logicalKey == LogicalKeyboardKey.arrowUp) {
         if (widget.onFocusUpEscape != null) {
-          AppLog.info('ChannelTile.onKeyEvent: calling onFocusUpEscape for '
-              '"${widget.channel.name}"');
           widget.onFocusUpEscape!.call();
           return KeyEventResult.handled;
         }
-        AppLog.info('ChannelTile.onKeyEvent: no escape, falling through to '
-            'default traversal for "${widget.channel.name}"');
+        FocusScope.of(context).focusInDirection(TraversalDirection.up);
+        return KeyEventResult.handled;
       }
       return KeyEventResult.ignored;
     };
@@ -168,21 +169,12 @@ class _ChannelTileState extends State<ChannelTile> {
       if (mounted) setState(() {});
       if (_focusNode.hasFocus) {
         _maybePrewarm();
-        // fix562 DIAGNOSTIC: log every focus-gain so the log shows the exact
-        // sequence of WHICH tile gained focus on each key press.
-        AppLog.info('ChannelTile: focus GAINED by "${widget.channel.name}" '
-            'nodeHash=${_focusNode.hashCode}');
         // fix560: requestFocus() alone does not scroll the focused widget
         // into view when it sits inside a shrink-wrapped, non-scrolling grid
         // nested in an ancestor Scrollable (the TV search results layout,
         // fix557). Mirrors the proven channel_schedule.dart pattern of
         // calling ensureVisible in a post-frame callback once the newly-
         // focused tile is actually built/laid out.
-        //
-        // fix562: REVERTED fix561's Duration.zero — the 3-second-gap test
-        // proved this was never a frame-timing race, so shortening the
-        // animation could not have been the real fix. Restored to 200ms
-        // pending the diagnostic data above.
         WidgetsBinding.instance.addPostFrameCallback((_) {
           if (!mounted || !_focusNode.hasFocus) return;
           final ctx = _focusNode.context;
diff --git a/lib/tv/tv_search_view.dart b/lib/tv/tv_search_view.dart
index 58ecf1c..ceed131 100644
--- a/lib/tv/tv_search_view.dart
+++ b/lib/tv/tv_search_view.dart
@@ -421,13 +421,6 @@ class _TvSearchViewState extends State<TvSearchView> {
                             final targetCol =
                                 col.clamp(0, prevLastRowNodes.length - 1);
                             final target = prevLastRowNodes[targetCol];
-                            // fix562 DIAGNOSTIC: confirm which node we're
-                            // targeting and its hash, to cross-reference
-                            // against ChannelTile's focus-gained log line.
-                            AppLog.info('TvSearch escape: from "$title" col=$col '
-                                '-> targetCol=$targetCol targetNodeHash='
-                                '${target.hashCode} targetHasFocus='
-                                '${target.hasFocus}');
                             target.requestFocus();
                           }
                         : null,
diff --git a/lib/whats_new_modal.dart b/lib/whats_new_modal.dart
index 0e53b79..33502cc 100644
--- a/lib/whats_new_modal.dart
+++ b/lib/whats_new_modal.dart
@@ -6,6 +6,9 @@ import 'package:open_tv/backend/settings_service.dart';
 /// in that minor". The dialog shows all entries for [version] whose key is
 /// a prefix of the running version string.
 const _changelog = <String, List<String>>{
+  '2.0.62': [
+    'TV: fixed search results D-pad navigation so the first Up press after jumping between sections (e.g. Channels → Movies) moves focus immediately instead of occasionally needing a second press.',
+  ],
   '2.0.61': [
     'Internal diagnostics only — no user-visible change. Added logging to track down the remaining search D-pad navigation quirk (the first Up after jumping sections sometimes needing a second press).',
   ],
diff --git a/pubspec.yaml b/pubspec.yaml
index 78f5994..813bed6 100644
--- a/pubspec.yaml
+++ b/pubspec.yaml
@@ -16,7 +16,7 @@ publish_to: "none" # Remove this line if you wish to publish to pub.dev
 # https://developer.apple.com/library/archive/documentation/General/Reference/InfoPlistKeyReference/Articles/CoreFoundationKeys.html
 # In Windows, build-name is used as the major, minor, and patch parts
 # of the product and file versions while build-number is used as the build suffix.
-version: 2.0.61+561
+version: 2.0.62+562
 
 environment:
   sdk: ^3.10.7
```

## Release — EXECUTE

```bash
git checkout main
git pull --ff-only
git apply fix562.md.diff   # or apply the diff block above
git add -A
git commit -m "fix562: TV search D-pad — first Up after a section-jump moves on press one (v2.0.62+562)"
git push origin main
bash scripts/build_and_release.sh v2.0.62
```
